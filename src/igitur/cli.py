...
def download_event_media_days(days: int, destination_path: Path | str):
    ...
    for event in events:
        ...
        event_path = destination_path / folder_name
...
def main():
    parser = argparse.ArgumentParser(
        prog="igitur", description="Command line interface to interact with Gaudeam."
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    # login comma
    login_parser = subparsers.add_parser(
        "login", help="Login to Gaudeam with email and password."
    )
    ...
    download_parser = subparsers.add_parser(
        "download", help="Download files from a Gaudeam folder."
    )
    download_parser.add_argument(
        "folder_id", help="ID of the Gaudeam folder to download from."
    )
    download_parser.add_argument(
        "destination", help="Destination directory to save files.", default="."
    )

    download_event_media_parser = subparsers.add_parser(
        "download-event-media", help="Download all media files from a Gaudeam event."
    )
    download_event_media_parser.add_argument(
        "event_id", help="ID of the Gaudeam event to download media from."
    )
    download_event_media_parser.add_argument(
        "destination", help="Destination directory to save files.", default="."
    )

    download_event_media_days_parser = subparsers.add_parser(
        "download-event-media-days",
        help="Download all media files from Gaudeam events in the last N days.",
    )
    download_event_media_days_parser.add_argument(
        "days", type=int, help="Number of days to look back for events.", default=14
    )
    download_event_media_days_parser.add_argument(
        "destination", help="Destination directory to save files.", default="."
    )
    ...
    upload_parser = subparsers.add_parser(
        "upload", help="Upload files or the content of a folder to a Gaudeam folder."
    )
    upload_parser.add_argument(
        "folder_id", help="ID of the Gaudeam folder to upload to."
    )
    upload_parser.add_argument("source", help="Path to the file or folder to upload.")

    upload_image_parser = subparsers.add_parser(
        "upload-images",
        help="Upload a folder and compress all images before uploading to a Gaudeam folder.",
    )
    upload_image_parser.add_argument(
        "folder_id", help="ID of the Gaudeam folder to upload to."
    )
    upload_image_parser.add_argument(
        "source", help="Path to the folder containing images to upload."
    )
